...
//  Floormap
//
//  Created by Matthew Newberry on 8/29/12.
...
static CGRect CGRectCenterInRect(CGRect rect, CGRect parentRect, BOOL horizontal, BOOL vertical) {
...
- (UIControl *)firstResponder
{
    ...
    for (UIView *subView in self.subviews) {
        ...
        if (firstResponder != nil) {
            return (UIControl *) firstResponder;
        }
    }
...
- (void)makeIntegralSubviews
{
    for(UIView *v in self.subviews) {
        v.frame = CGRectIntegralScaled(v.frame);
        
        if([v.subviews count] > 0) {
            [v makeIntegralSubviews];
        }
    }
}
